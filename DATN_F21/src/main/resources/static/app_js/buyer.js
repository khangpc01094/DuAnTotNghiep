...
app.controller("buyer-ctrl", function($scope, $http) {
    ...
    $scope.regis = function () {
        ...
        $http.post(`/rest/buyer/regis`, item).then(resp => {
            $scope.reset();
            return Swal.fire({
                width: '400px',
    ...
    $scope.deleteaddress = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $http.delete(`/rest/address/buyer/del_address/${id}`).then(resp => {
    ...
    $scope.updateaddress = function () {
        var item = angular.copy($scope.form);
        $http.put(`/rest/address/buyer/edit_address`, item).then(resp => {
            // $scope.reset();
    ...
    $scope.add_address = function() {
        var item = angular.copy($scope.form);
        $http.post(`/rest/address/buyer/add_address`, item).then(resp => {
            $scope.reset();
            return Swal.fire({
                width: '400px',
                title: 'Thêm địa chỉ thành công!',
