...
app.controller("seller-ctrl", function($scope, $http) {
    ...
    $scope.findCates = function() {
        ...
        var name1 = $('#product').val();
        var sid1 = $('#sid').val();
        $http.get(`http://localhost:8080/rest/products/category/${cate}/${sid1}`).then(resp => {
            $scope.items = resp.data;
            console.log("items find: ", resp.data);
        });
    };
...
app.controller("update-seller-ctrl", function($scope, $http) {
    ...
    $scope.loadImages = function() {
        ...
        var pid = $('#pid').val();
        console.log('pid: ' + pid);
        $http.get(`rest/proimg/images/${pid}`).then(resp => {
            $scope.images = resp.data;
            console.log('images loaded: ', resp.data);
        });
    };

    $scope.productImage = function() {
        var sid = $('#sid').val();
        var pid = $('#pid').val();
        console.log('pid: ' + pid);
        $http.get(`http://localhost:8080/rest/proimg/${pid}/${sid}`).then(resp => {
            $scope.productImage = resp.data;
            console.log('images loaded: ', resp.data);
        });
    };
    ...
    //hiển thị lên form
    $scope.edit = function(item) {
        $scope.productImage = angular.copy(item);
        $scope.productImage();
        // $("#pills-tab button:eq(0)").tab('show');
    };
    ...
    $scope.pager = {
        page: 0,
        size: 5,
        get productImage() { //lọc sp theo trang
            var start = this.page * this.size; //vị trí bắt đầu
            return $scope.productImage.slice(start, start + this.size); //slice để cắt
        },
        get count() { //tổng số trang
            return Math.ceil(1.0 * $scope.productImage.length / this.size); //tổng số phần tử chia kích thước 1 trang và làm tròn
        },
        first() {
            this.page = 0;
        },
